...
async fn submit_claim_to_miden(
    ...
    let result = tokio::task::spawn_blocking(move || {
        runtime_handle.block_on(async {
            ...
            info!(ephemeral_account_id = %ephemeral_account_id, "Ephemeral account created");
            ...
            let sync_after_ephemeral = client.sync_state().await
                .map_err(|e| ClientError::SyncError(e.to_string()))?;
            debug!(block_num = sync_after_ephemeral.block_num.as_u32(), "Sync complete after adding ephemeral account");

            let submitter_account_id = ephemeral_account_id;

            // Step 6: Create agglayer faucet (local only, not deployed to network)
            ...
            info!("Preparing BridgeClaimParams for CLAIM note creation");
            ...
            info!(
                ...
                amount = claim_data.amount,
                "Submitting CLAIM transaction to network"
            );

            // Step 10: Submit the transaction from the ephemeral account
            ...
            info!(
                ...
                block_num = block_num,
                "CLAIM note submission complete"
            );
