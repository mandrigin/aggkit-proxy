...
clean_bridge_db() {
    ...
    # Full clean of bridge DB - truncate all sync tables
    # The bridge will re-index everything from scratch
    docker exec "$postgres_container" psql -U bridge_user -d bridge_db -c "
        TRUNCATE sync.exit_root CASCADE;
        TRUNCATE sync.block CASCADE;
        TRUNCATE sync.deposit CASCADE;
        TRUNCATE sync.claim CASCADE;
        TRUNCATE sync.monitored_txs CASCADE;
        TRUNCATE sync.monitored_txs_group CASCADE;
    " 2>/dev/null && success "Cleaned bridge DB (full truncate)" || warn "Failed to clean bridge DB (tables may not exist yet)"
}
